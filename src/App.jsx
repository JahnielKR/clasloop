...
import { C as ThemeC } from './components/tokens';
...
function AuthScreen({ initialMode = "select", initialRole = "teacher", onBack }) {
  ...
  const inp = { fontFamily: "'Outfit',sans-serif", background: ThemeC.bg, border: `1px solid ${ThemeC.border}`, color: ThemeC.text, padding: "11px 14px", borderRadius: 8, fontSize: 14, width: "100%", outline: "none" };
  const btnP = { width: "100%", padding: "12px", borderRadius: 9, fontSize: 15, fontWeight: 600, background: `linear-gradient(135deg,${ThemeC.accent},${ThemeC.purple})`, color: "#fff", border: "none", cursor: "pointer", opacity: loading ? .5 : 1, fontFamily: "'Outfit',sans-serif" };
  const btnS = { width: "100%", padding: "12px", borderRadius: 9, fontSize: 15, fontWeight: 600, background: ThemeC.bg, color: ThemeC.text, border: `1px solid ${ThemeC.border}`, cursor: "pointer", fontFamily: "'Outfit',sans-serif" };

  if (mode === "select") return (
    <div style={{ minHeight: "100vh", background: ThemeC.bgSoft, display: "flex", alignItems: "center", justifyContent: "center", padding: 24 }}>
      <div style={{ maxWidth: 400, width: "100%", textAlign: "center" }}>
        <div style={{ display: "flex", justifyContent: "center", marginBottom: 16 }}>
          <LogoMark size={48} />
        </div>
        <h1 style={{ fontFamily: "'Outfit'", fontSize: 26, fontWeight: 700, marginBottom: 6 }}>Welcome to Clasloop</h1>
        <p style={{ color: ThemeC.textSecondary, fontSize: 15, marginBottom: 32 }}>Help your students actually remember what you teach</p>
        ...
        <p style={{ marginTop: 24, fontSize: 13, color: ThemeC.textMuted, fontFamily: "'Outfit'" }}>Already have an account? <span onClick={() => setMode("login")} style={{ color: ThemeC.accent, cursor: "pointer", fontWeight: 500 }}>Sign in</span></p>
      </div>
    </div>
  );
  ...
  return (
    <div style={{ minHeight: "100vh", background: ThemeC.bgSoft, display: "flex", alignItems: "center", justifyContent: "center", padding: 24 }}>
      <div style={{ maxWidth: 400, width: "100%" }}>
        ...
        }} style={{ background: "transparent", border: "none", color: ThemeC.textSecondary, fontSize: 13, cursor: "pointer", marginBottom: 16, fontFamily: "'Outfit'" }}>← Back</button>
        <div style={{ background: ThemeC.bg, borderRadius: 14, border: `1px solid ${ThemeC.border}`, padding: 28 }}>
          <h2 style={{ fontFamily: "'Outfit'", fontSize: 22, fontWeight: 700, marginBottom: 4 }}>{mode === "signup" ? "Create your account" : "Welcome back"}</h2>
          <p style={{ fontSize: 13, color: ThemeC.textSecondary, marginBottom: 20, fontFamily: "'Outfit'", display: "flex", alignItems: "center", gap: 6 }}>{mode === "signup" ? <>{role === "teacher" ? <><TeacherInline size={16}/> Teacher account</> : <><StudentInline size={16}/> Student account</>}</> : "Sign in to your account"}</p>
          {error && <div style={{ padding: "10px 14px", borderRadius: 8, background: ThemeC.redSoft, color: ThemeC.red, fontSize: 13, marginBottom: 14, fontFamily: "'Outfit'" }}>{error}</div>}
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 16 }}><div style={{ flex: 1, height: 1, background: ThemeC.border }}/><span style={{ fontSize: 12, color: ThemeC.textMuted }}>or</span><div style={{ flex: 1, height: 1, background: ThemeC.border }}/></div>
          <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
            {mode === "signup" && <div><label style={{ display: "block", fontSize: 13, fontWeight: 500, color: ThemeC.textSecondary, marginBottom: 5, fontFamily: "'Outfit'" }}>Full name</label><input value={name} onChange={e => setName(e.target.value)} placeholder="Your name" style={inp}/></div>}
            <div><label style={{ display: "block", fontSize: 13, fontWeight: 500, color: ThemeC.textSecondary, marginBottom: 5, fontFamily: "'Outfit'" }}>Email</label><input type="email" value={email} onChange={e => setEmail(e.target.value)} placeholder="you@school.edu" style={inp}/></div>
            <div><label style={{ display: "block", fontSize: 13, fontWeight: 500, color: ThemeC.textSecondary, marginBottom: 5, fontFamily: "'Outfit'" }}>Password</label><input type="password" value={pass} onChange={e => setPass(e.target.value)} placeholder={mode === "signup" ? "At least 8 characters" : "Your password"} style={inp} onKeyDown={e => e.key === "Enter" && (mode === "signup" ? handleSignup() : handleLogin())}/></div>
          </div>
          ...
          <p style={{ textAlign: "center", marginTop: 14, fontSize: 13, color: ThemeC.textMuted, fontFamily: "'Outfit'" }}>
            {mode === "signup" ? <>Already have an account? <span onClick={() => { setMode("login"); setError(""); }} style={{ color: ThemeC.accent, cursor: "pointer", fontWeight: 500 }}>Sign in</span></> : <>Don't have an account? <span onClick={() => { setMode("signup"); setError(""); }} style={{ color: ThemeC.accent, cursor: "pointer", fontWeight: 500 }}>Sign up</span></>}
          </p>
        </div>
      </div>
    </div>
