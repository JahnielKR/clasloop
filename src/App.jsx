...
// ── PR 28.17.2: blocker shown when a mobile (phone) user lands on a
//   page that genuinely doesn't fit a small screen. Clasloop is a
//   classroom tool — students typically use tablets or computers in
//   class. Building a fully phone-friendly quiz layout would be 30+
//   hours of theme rework for a small slice of usage. Instead, we let
//   phones browse the app (read MyClasses, Favorites, Settings, etc.)
//   but block the interactive flows that require landscape: practice
//   mode and the join-session quiz.
//
// The user can still log in, see their classes, browse — just can't
// "play" a session on the phone. They get a friendly screen telling
// them to switch device.
function MobileBlockedScreen({ lang = "en", onBack }) {
  const txt = {
    en: {
